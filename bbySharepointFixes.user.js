// ==UserScript==
// @name           BBY Sharepoint Fixes
// @namespace      http://userscripts.org/users/mathiasaurusrex
// @description    Reloads after every 3 min of idle time to prevent logout. Fixes a Google Chrome hover problem. Adds automcomplete to login form. 
// @include        https://view.bestbuy.com/*
// @match          https://view.bestbuy.com/*
// @downloadURL    http://userscripts.org/scripts/source/155512.user.js
// @version 1.1.13
// ==/UserScript==



	var minutes = 3;
	var timeOutPeriod = 1000*60*minutes;
	var tId; 

	function setRedirect() {
	    window.location = window.location;
	}
	
	tId = setTimeout(setRedirect, timeOutPeriod);

	function resetTimer() {
	    clearTimeout(tId);
	    tId = setTimeout(setRedirect, timeOutPeriod);
	}


	/*window.onload = playAudio;
	function playAudio(){
		var link = "https://mathiasrechtzigel.com/music/RickyMartin_TheCupOfLife.mp3";
		var iframe = document.createElement('iframe');
		iframe.frameBorder=0;
		iframe.width="10px";
		iframe.height="10px";
		iframe.id="randomid";
		iframe.setAttribute("src", link);
		document.body.appendChild(iframe);
	}*/
	
	window.addEventListener( 'mousemove', resetTimer, true );
	window.addEventListener( 'scroll', resetTimer, true );
	
	/* Counts number of Assigned tasks and updates the title bar */	
	function updateTitleToAssigned() {
		var splitBySearchWord = (document.body.textContent).split('Assigned');
		var numOfAssigned = splitBySearchWord.length;	
		document.title = numOfAssigned-2 + ' Assigned Ticket(s)';
	}
		window.setInterval(updateTitleToAssigned, 1200);
	
	/* Fixes hover problem in Chrome */
	GM_addStyle('body{position:absolute;width:100%;overflow-x:hidden;background:url(http://i.imgur.com/cJNnT2B.png) no-repeat bottom right fixed, -webkit-gradient(linear, left top, left bottom, color-stop(0%,#cbeff8), color-stop(100%,#175485)) fixed !important}');
	
	/* Enable autocomplete for login form */
	var username =  document.getElementById('username');
	if (typeof(username) != 'undefined' && username != null) {
  		username.setAttribute('autocomplete', 'on');
	}
		
	/* Add a unique Sharepoint favicon */
	addIcon();
	function addIcon() {
		var newIcon = document.createElement('link');
		newIcon.type = 'image/x-icon';
		newIcon.rel = 'shortcut icon';
		newIcon.href = 'data:image/png;base64,';
		this.document.getElementsByTagName('head')[0].appendChild(newIcon);
	}

/*Embed Music - Ricky Martin Cup of Life 
	window.onload = function(){
	var link = "http://mathiasrechtzigel.com/music/RickyMartin_TheCupOfLife.mp3";
	var iframe = document.createElement('iframe');
	iframe.frameBorder=0;
	iframe.width="10px";
	iframe.height="10px";
	iframe.setAttribute("src", link);
	document.body.appendChild(iframe);
};

window.onload();*/

